...
class Vestas_Report(_Service_Report):
    ...
    def extract_raw_checklist(self) -> pd.DataFrame:
        """
        Extracts the raw inspection checklist table from the PDF report.
        ...
        Returns:
            pd.DataFrame: The raw inspection checklist table before any formatting

        Raises:
            ValueError: If no 'Service Inspection Form' page is found
        """
    ...
    def get_inspection_checklist_df(self) -> pd.DataFrame:
        """
        Returns the complete formatted inspection checklist.

        This method:
        1. Extracts the raw checklist data
        2. Applies formatting pipeline to clean and standardize the data

        Returns:
            pd.DataFrame: Fully processed inspection checklist
        """
        raw_checklist = self.extract_raw_checklist()
        formatted_checklist = self.format_table(raw_checklist)
...
#         """
#         def process_report(self) -> tuple[pd.DataFrame, pd.DataFrame]:
#             """
#             Process the report to extract metadata and inspection checklist.
    
#             This method:
#             1. Extracts metadata from the report
#             2. Extracts and formats the inspection checklist
    
#             Returns:
#                 tuple: A tuple containing the metadata DataFrame and the formatted inspection checklist DataFrame
#             """
#             # Get metadata
...
#             return metadata, formatted_inspection
