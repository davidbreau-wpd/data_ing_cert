...
######## METADATA HANDLING
    ...
    def _set_order_type(self) -> None:
        ...
        Attributes:
            order_type(str): Reason maintenance is ordered
        """
        try:
            order_type = self.pages[0].split('\n')[0].strip()
            self.order_type = order_type if order_type else None
        except Exception as e:
            logger.error(f"Failed to extract order type: {str(e)}")
...
######## INSPECTIONS CHECKLIST HANDLING
    ...
    def _extract_raw_checklist(self) -> None:
        ...
        try:
            ...
            logger.error(f"Error extracting raw checklist: {e}")
