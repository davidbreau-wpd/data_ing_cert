...
# import polars as pl
# # import pandas as pd # Remove pandas import if not needed elsewhere

# # Assuming this method is within a class like _Service_Report_Parser or CSVFormatter

# def _stack_columns_in_pairs_polars(self, df: pl.DataFrame) -> pl.DataFrame:
#     """
#     Description:
#         Stacks Polars DataFrame columns in pairs, renaming them to 0 and 1 respectively.
#         Adapted for Polars' requirement of unique column names.

#     Args:
#         df (pl.DataFrame): The Polars DataFrame to process. Should have an even number
#                            of columns for proper pairing.

#     Returns:
#         pl.DataFrame: A new DataFrame with columns stacked in pairs and renamed
#                       to 0 and 1.
#     """
#     # Input validation (optional but good practice)
#     if df.shape[1] % 2 != 0:
#         raise ValueError("DataFrame must have an even number of columns for pairing.")

#     stacked_pairs = []

#     # Iterate through columns in steps of 2
#     for i in range(0, df.shape[1], 2):
#         # Select the pair of columns by index.
#         # Use .select() with pl.col() to get a new DataFrame with the selected columns.
#         pair_df = df.select(pl.col(df.columns[i:i+2]))

#         # Rename the columns of the pair to 0 and 1.
#         # This renaming only applies to this specific pair DataFrame before appending.
#         pair_df = pair_df.rename({pair_df.columns[0]: "0", pair_df.columns[1]: "1"})

#         stacked_pairs.append(pair_df)

#     # Concatenate the list of pair DataFrames.
#     # Each DataFrame in the list now has unique column names ("0" and "1").
#     result = pl.concat(stacked_pairs, how='vertical') # Use how='vertical' for stacking rows
...
#     return result
    ...
    def merge_rows_by_capitalization(self, df: pd.DataFrame, new_line: bool = False) -> pd.DataFrame:
        ...
        Example:
            ...
            Output DataFrame (new_line=False):
                0    1                  2
                1    Header Cont.      Result1 Result2
        """
        result = []
        current_row = None
    ...
    def _merge_dataframes(self, main_df: pd.DataFrame, overview_df: pd.DataFrame, columns_to_keep: list[int] = None) -> pd.DataFrame:
        """
        Merge two DataFrames while optionally selecting specific columns from the second DataFrame.
        
        Args:
            main_df: Primary DataFrame to keep intact
            overview_df: Secondary DataFrame to merge
            columns_to_keep: Optional list of column indices to select from overview_df
